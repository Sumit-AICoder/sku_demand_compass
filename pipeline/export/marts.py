...
def build() -> None:
    # Every rollup below groups by product_line. It is not optional: a totals table that
    # lost the key would add a 7-lakh tractor to a 42k cultivator and call the result units.
    ...
    for level, keys, geo in [
        ("block", ["block_id", "product_line", "sku_id", "category"], b),
        ("district", ["district_id", "product_line", "sku_id", "category"], d),
    ]:
    ...
    state_sku = sc.merge(v[["village_id", "state"]], on="village_id").groupby(
        ["state", "product_line", "sku_id", "category"], observed=True).agg(
    ...
    for level, key, geo, namecol in [
        ...
        gk = [key, "product_line"]
        tot = sc.groupby(gk, observed=True).agg(
            **{m: (m, "sum") for m in MEASURES}).reset_index()
        # The best-selling SKU is per line too -- otherwise every village's "top product"
        # becomes a tractor, because one tractor outsells any implement by value.
        top = (sc.sort_values("potential_units_yr", ascending=False)
               .groupby(gk, observed=True).head(1)[gk + ["sku_id", "category"]]
               .rename(columns={"sku_id": "top_sku", "category": "top_category"}))
        tot = tot.merge(top, on=gk, how="left").merge(geo, on=key, how="left")
