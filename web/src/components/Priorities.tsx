...
export default function Priorities() {
  ...
  const pri = useAsync(() => api.planPriorities(state, productLine), [state, productLine])
    ...
    <div className="grid" style={{ gap: 16 }}>
      ...
      <div className="split">
        <Card title="Focus products" note="demand potential × real subsidy lever">
          <div className="filters" style={{ margin: '0 0 10px', marginLeft: 0 }}>
            <label className="dim" style={{ fontSize: 12, alignSelf: 'center' }}>Subsidy state:</label>
            <select value={state} onChange={e => setState(e.target.value)}>
              {STATES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
          <Async state={pri}>{(d: any) => {
            const rows = d.skus as any[]
            const maxU = Math.max(1, ...rows.map(r => r.units || 0))
            return (
              <table>
                <thead><tr>
                  <th>Product</th>
                ...
                <tbody>
                  {rows.slice(0, 18).map(r => {
                    const push = (r.units >= maxU * 0.25) && r.subsidy_pct >= 40
                    return (
        ...
        <Card title="District priorities vs real cropland"
              note="DES cropped area · demand intensity per '000 ha">
          <Async state={dist}>{(d: any) => {
            const rows = (d.districts as any[]).filter(r => r.crop_area_lha)
            return (
              <table>
                <thead><tr>
